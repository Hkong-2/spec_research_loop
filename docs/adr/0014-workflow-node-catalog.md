...
`loop` creates a Node Head per Workflow Node when a Loop Session is born. The catalog is: `idea_interpretation`, `idea_decomposition`, `research_inputs`, `related_work`, `gap`, `contribution`, `claims`, `evidence`, `experiment_plan`, `feasibility`, `gap_judge`, `contribution_judge`, `evidence_judge`, `experiment_judge`, `conference_judge`, `aggregator`. Readiness is a Loop Stage with no Node Head. Confirming `feasibility` mints a Spec Version; `spec_version` is not a Workflow Node. Catalog, Loop Stage groupings, and invalidation edges are constants in `loop`, not rows.
